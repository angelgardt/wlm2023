...
taia %>% 
  select(id, matches("[[:alpha:]]{2}\\d{2}"),
         n_dighelp, f_socnet, dig_job, dig_spec) %>% 
  write_csv("../data/pr9/taia.csv")
```
...
a. Исследуйте переменные.
b. Какой коэффициент корреляции здесь лучше всего использовать?
c. Проведите статистический тест и проинтерпретируйте результаты.
...
original$scale <- colnames(original)
original %>% write_csv("../data/pr9/original_cormat.csv")
```
...
Проверьте, отличается ли матрица `mr_pooled_cor` от [корреляционной матрицы оригинального исследования](https://raw.githubusercontent.com/angelgardt/wlm2023/master/data/pr9/original_cormat.csv).
