...
fs %>% 
  select(-X) %>% 
  filter(id == 1 | id == 2) %>% 
  write_csv2("../data/pr3/des_fs_1.csv")
i=5
fs %>% 
  select(-X) %>% 
  filter(id == i) %>% 
  write_csv2(paste0("../data/pr3/p3-data/des_fs_", i, ".csv"))
...
`readLines()` поможет загрузить несколько строк, чтобы посмотреть, какова структура файла.
...
::: {#pr3-6-task .task}
a. Мы знаем, что файлы данных считываются функциями типа `read...()` в датафрейм. Но давайте в этом дополнительны убедимся. Проверьте, какой структурой данных является объект `fs`.
