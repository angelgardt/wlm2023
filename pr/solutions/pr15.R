...
# 1
str(iris)
## sepals --- чашелистики, petals --- лепестки
...
# 4
## a
hc_complete_cl2 <- cutree(hc_complete, 2)
GGally::ggpairs(iris, 
                columns = 1:4, 
                aes(color = Species, 
...
## b
hc_complete_cl3 <- cutree(hc_complete, 3)
GGally::ggpairs(iris, 
                columns = 1:4, 
                aes(color = Species, 
...
hc_complete_cl4 <- cutree(hc_complete, 4)
GGally::ggpairs(iris, 
                columns = 1:4, 
                aes(color = Species, 
...
## b
hc_single_cl2 <- cutree(hc_single, 2)
GGally::ggpairs(iris, 
                columns = 1:4, 
                aes(color = Species, 
...
## b
hc_average_cl2 <- cutree(hc_average, 2)
GGally::ggpairs(iris, 
                columns = 1:4, 
                aes(color = Species, 
...
## data for graph
clust.stats <- cstats.table(d, hc_complete, 10)
ggplot(data = data.frame(t(clust.stats)),
       aes(x = cluster.number, y = within.cluster.ss)) +
  geom_point() +
  geom_line() +
  ggtitle("Hierarchical (conplete)") +
...
# 11
fviz_nbclust(x = iris[1:4], FUNcluster = kmeans, diss = d, method = "silhouette", k.max = 10)
...
hc_cpi_complete <- hclust(d_cpi, method = "compl")
hc_cpi_single <- hclust(d_cpi, method = "sing")
hc_cpi_ave <- hclust(d_cpi, method = "ave")
...
# 14
fviz_nbclust(x = cpi[1:10], FUNcluster = fanny, diss = d_cpi, method = "silhouette", k.max = 15, memb.exp = 1.5)
fviz_nbclust(x = cpi[1:10], FUNcluster = fanny, diss = d_cpi, method = "silhouette", k.max = 15, memb.exp = 1.3)


# 15
cpi_fuzzy <- fanny(x = d_cpi, k = 2, memb.exp = 1.3)
...
# 16
fviz_silhouette(silhouette(x = cpi_fuzzy$clustering,
                           dist = d_cpi))
...
fviz_cluster(object = list(data = d_cpi,
             ...
             geom = "point")
cpi_fuzzy2$clustering


# 18
fviz_nbclust(x = iris[1:4], FUNcluster = fanny, diss = d, method = "silhouette", k.max = 10, memb.exp = 1.5)
