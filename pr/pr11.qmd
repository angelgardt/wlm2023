...
Сегодня нам понадобятся пакеты `car`, `ez` и `multcomp`. Проверьте, какие из них установлены на вашем компьютере, если какие-либо отсутствуют, установите их.

Можно совместить проверку и установку c помощью следующего кода:

```{r, echo=TRUE}
pkgs <- c("car", "ez", "multcomp")
...
c. Отберите из данных корректные пробы с одним целевым стимулом. Перезапишите объект `share`.
:::
...
Аггрегируйте данные по времени реакции (`time1`) с учетом всех (квази)экспериментальных условий, то есть всех переменных
:::
