export class Handler {
  ...
  jsonHandler(info_json, N_TASKS=this.N_TASKS, ID=this.ID) {
    let info = {};
  ...
  showHints(id, counter = this.counter) {
    if (counter[id] > 2) {
  ...
  check(id) {
    this.checker(id = id, ans = this.info["autocheck_answer"][id]);
  }
